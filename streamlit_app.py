...
# Google Drive 파일 ID
file_id = '153AZU7ArXTK3mY9meEk-jTVyJvAJ2oaH'
...
def display_right_content(labels):
    ...
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.image(f"https://i.ibb.co/6scjk4m/1.jpg?text={label}", caption=f"이미지: {label}", use_column_width=True)

    # 2nd Row - YouTube Videos based on labels
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.video("https://youtu.be/veAll04qC4M?feature=shared", start_time=0)
            st.caption(f"유튜브: {label}")

    # 3rd Row - Text based on labels
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.write(f"{label} 관련 텍스트 내용입니다. 콘치즈 먹고싶당 우헤헤")
